    ...
    return (
        <div className="max-w-lg mx-auto mt-10 p-8 bg-white dark:bg-slate-800 rounded-lg shadow-xl text-center">
            ...
            <p className="text-slate-700 dark:text-slate-300 text-lg">Your RSVP has been recorded as: <strong className="font-semibold">{submittedStatus}</strong>.</p>
             {event.photoAlbumUrl && (
              <a href={event.photoAlbumUrl} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex items-center justify-center gap-2 bg-rose-500 text-white font-bold py-3 px-5 rounded-lg shadow-md hover:bg-rose-600 transition duration-300">
                <PhotoIcon className="h-5 w-5" />
                <span>View & Add Photos</span>
              </a>
            )}
        </div>
    );
    ...
    <div className="max-w-2xl mx-auto p-4 sm:p-6">
      <div className="bg-white dark:bg-slate-800 p-8 rounded-lg shadow-xl">
        ...
        <div className="mt-6 space-y-4 border-t border-slate-200 dark:border-slate-700 pt-6">
            ...
            {event.photoAlbumUrl && (
              <div className="pt-4">
                <a href={event.photoAlbumUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-rose-50 text-rose-600 dark:bg-rose-900/50 dark:text-rose-300 font-semibold py-3 px-4 rounded-lg shadow-sm hover:bg-rose-100 dark:hover:bg-rose-900 transition duration-300 w-full">
                  <PhotoIcon className="h-5 w-5" />
                  <span>View & Add to Shared Album</span>
                </a>
              </div>
            )}
        </div>
